...
start_worker() {
  ...
  local pid=$!
  WORKER_PIDS+=("${pid}")
  echo "[weekly] worker ${label} pid=${pid}"
}
...
wait_for_queue_drain() {
  ...
  while ((SECONDS < deadline)); do
    ...
    echo "[weekly] queue pending=${pending}"
    if [[ "${pending}" == "0" ]]; then
      return 0
    fi
    sleep "${poll_sec}"
  done
  echo "[weekly] warning: queue drain timeout pending=${pending}" >&2
  return 1
}
...
echo "[weekly] 1) start queue producer (sync)"
run_substudy sync \
  ...
  "${NETWORK_ARGS[@]}" &
SYNC_PID=$!
echo "[weekly] sync producer pid=${SYNC_PID}"

echo "[weekly] 2) start queue workers"
...
echo "[weekly] 3) wait sync producer"
wait "${SYNC_PID}"
SYNC_PID=""
