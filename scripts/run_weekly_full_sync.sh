...
echo "[weekly] 1) sync start (network lane)"
run_substudy sync \
  --skip-ledger \
  --config "${CONFIG_PATH}" \
  --ledger-db "${LEDGER_DB}" \
...
cpu_lane() {
  echo "[weekly] 2-A) CPU lane start (asr/loudness)"
  ...
  echo "[weekly] 2-A) CPU lane done"
}

mem_lane() {
  echo "[weekly] 2-B) MEM lane start (translate-local)"
  while kill -0 "${SYNC_PID}" 2>/dev/null; do
...
echo "[weekly] 3) wait sync"
wait "${SYNC_PID}"

echo "[weekly] 4) wait workers"
wait "${CPU_PID}" || true
wait "${MEM_PID}" || true

echo "[weekly] 5) final catch-up"
