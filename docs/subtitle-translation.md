...
3. **バッチ処理の標準フロー（5本並列書き込み）**

   大量バッチ（5本以上）の場合は以下の順序で処理する:

   1. **ソース全件を並列読み込み**（10本ずつ並列 Read）
   2. **翻訳を5本ずつ並列書き込み**（Write を5本同時実行 × 必要回数）
   3. **全件まとめて検証**（キュー数・時刻一致）
   4. **全件まとめてDB記録**

   > **5本並列が標準。** コンテキスト効率と誤り発見のバランスが最も良い。
   > ソースをまず全部読んでからWriteに入ることで、並列書き込み時に都度ファイルを開く必要がなくなる。
...
```
   ...
   **3c.** 検証OKなら即座に翻訳ログへ記録する（`master_ledger.sqlite` の `translation_runs` テーブル）。
   ...
   > **重要:** 標準フロー（5本並列）では全件書き終えてから一括検証・一括DB記録する。少量・復旧時の逐次フローでは、DB記録をバッチ末尾にまとめず1件ごとに行うことで中断耐性を確保する。
...
実行手順:
- **5本ずつ並列 Write** を繰り返し、全件書き終えてから一括検証・一括DB記録する（標準フロー）
