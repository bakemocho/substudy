...
3. **バッチ処理の標準フロー（5本単位の読み書きセット）**

   大量バッチ（5本以上）の場合は **5本ずつ読み書きをセットで繰り返す**:

   1. **ソース5本を並列読み込み**（Read を5本同時）
   2. **翻訳を5本並列書き込み**（Write を5本同時）
   3. 次の5本へ進み、1〜2を繰り返す
   4. **全件書き終えてから一括検証**（キュー数・時刻一致）
   5. **全件まとめてDB記録**

   > **なぜ「全件先読み → 後でWrite」ではなく「5本ずつ読み書き」なのか:**
   > コンテキスト再開直後や圧縮後のセッションでは残量が少ない。全30件を先に読むとソース全文がコンテキストを埋め尽くし、書き込み前に溢れる。5本単位で読み書きすればコンテキスト内に保持するソース量を常に小さく抑えられる。

   少量（1〜4本）の場合や中断復旧時は、以下の旧スタイルも使える:
   バッチ内の各ファイルについて「3a翻訳 → 3b検証 → 3c記録」を1件ずつ繰り返す。
...
```
   ...
   **3c.** 検証OKなら即座に翻訳ログへ記録する（`master_ledger.sqlite` の `translation_runs` テーブル）。
   ...
   > **重要:** 標準フロー（5本単位の読み書きセット）では全件書き終えてから一括検証・一括DB記録する。少量・復旧時の逐次フローでは、DB記録をバッチ末尾にまとめず1件ごとに行うことで中断耐性を確保する。
...
実行手順:
- **5本ずつ Read → Write をセットで繰り返し**、全件書き終えてから一括検証・一括DB記録する（標準フロー）
- 少量や復旧時は1件ごとに「翻訳 → 検証 → DB記録」でもよい
